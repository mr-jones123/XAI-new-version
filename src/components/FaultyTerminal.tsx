...
export default function FaultyTerminal({
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    if (isMobileRef.current) return; // Skip on mobile devices
  ...
  const handleTouchEnd = useCallback(() => {
    if (!mouseReact) return;
    // Reset position after a delay on mobile
  ...
  useEffect(() => {
    ...
    const adjustedDPR = isMobileRef.current ? Math.min(dpr, 1.5) : dpr;
    const renderer = new Renderer({ dpr: adjustedDPR });
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uScale: { value: isMobileRef.current ? scale * 0.8 : scale }, // Slightly smaller scale on mobile

        uGridMul: { value: new Float32Array(isMobileRef.current ? [gridMul[0] * 0.8, gridMul[1] * 0.8] : gridMul) },
    ...
    if (mouseReact) {
      if (isMobileRef.current) {
        ...
        ctn.addEventListener("touchend", handleTouchEnd, { passive: true });
      } else {
